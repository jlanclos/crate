...
TEST(messageMethods, messageConstruction) {
  ...
  EXPECT_EQ(mockNewMessage.getIdAdd().getArray(),
            mac(0x00, 0x00, 0x00, 0x00, 0x00, 0x00).getArray());
...
TEST(messageMethods, messageSetGetIdAdd) {
  ...
  EXPECT_EQ(mockNewMessage.getIdAdd().getArray(),
            mac(0x23, 0x34, 0xA3, 0x76, 0x43, 0x88).getArray());
}
...
TEST(messageMethods, messageSetGetPayload) {
  ...
  EXPECT_EQ(mockNewMessage.getPayload()[6].pullIp().second.getArray(),
            ip(192, 168, 10, 112).getArray());
}
...
TEST(messageQueueMethods, messageQueuePushPopReadPurge) {
  ...
  mockNewMessageQueue.push(idType::DATABLOCK_UPDATE, mac(0x23, 0x43, 0xA3, 0x89, 0x12, 0x45),
                           mockPayload1);
  mockNewMessageQueue.push(idType::CONFIGBLOCK_REPORT, mac(0x34, 0x65, 0xB4, 0x78, 0x35, 0x90),
                           mockPayload2);
  ...
  EXPECT_EQ(mockNewMessageQueue.read().second.getPayload()[0].pullString().second,
            "This is a string");
