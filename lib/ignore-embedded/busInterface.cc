...
void BUS::readSensors() {
  ...
  this->humidityRaw = (((aht20Raw[1] << 16 | aht20Raw[2] << 8 | aht20Raw[3]) >> 4) / pow(2, 20)) * 100.0;
  this->temperatureRaw =
      (((aht20Raw[3] << 16 | aht20Raw[4] << 8 | aht20Raw[5]) & ~(0xFFF00000)) / pow(2, 20)) * 200 - 50;
...
void BUS::writeData(uint8_t address, uint8_t pointer, uint8_t sendDelay, std::vector<uint8_t> data) {
  Wire.beginTransmission(address);
  Wire.write(pointer);
...
std::vector<uint8_t> BUS::readData(uint8_t address, uint8_t pointer, uint8_t responseDelay, uint8_t bytes) {
  Wire.beginTransmission(address);
  Wire.write(pointer);
  Wire.endTransmission();
  delay(responseDelay);
